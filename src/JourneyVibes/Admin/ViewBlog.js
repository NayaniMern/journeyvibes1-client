...
const ViewBlogs = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://journeyvibes1-server.onrender.com/blogs`)
  ...
  const deleteBlogs = (blogsId) => {
    axios
      .delete(`https://journeyvibes1-server.onrender.com/blogs/${blogsId}`)
      .then(() => alert("Your Selected Blog was Deleted"))
      .catch((err) => console.log(err));
  };
  const getOneRecord = (blogsId) => {
    axios
      .get(`https://journeyvibes1-server.onrender.com/blogs/${blogsId}`)
      .then((res) => {
  ...
  const updateEnq = (e) => {
    e.preventDefault();
    axios
      .put(`https://journeyvibes1-server.onrender.com/blogs/${_id}`, {
        _id,
        image,
        name,
        des,
