...
const ViewServices = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://journeyvibes1-server.onrender.com/services`)
  ...
  const deleteServices = (servicesId) => {
    axios
      .delete(`https://journeyvibes1-server.onrender.com/services/${servicesId}`)
      .then(() => alert("Your Selected Service was Deleted"))
      .catch((err) => console.log(err));
  };
  const getOneRecord = (servicesId) => {
    axios.get(`https://journeyvibes1-server.onrender.com/services/${servicesId}`).then((res) => {
  ...
  const updateEnq =(e)=>{
    e.preventDefault()
    axios.put(`https://journeyvibes1-server.onrender.com/services/${_id}`,{_id,image,name,description})
